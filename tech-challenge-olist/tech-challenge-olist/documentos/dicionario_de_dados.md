...
Documentação de referência das 9 tabelas do dataset e das decisões técnicas do
notebook de compreensão dos dados. Fonte: [Kaggle — Brazilian E-Commerce Public Dataset
by Olist](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce).
...
Primeiro esboço de resposta à pergunta norteadora, para apresentar ao grupo na
Sincronização 1 (Dia 5):
...
| Outliers | Documentado no Sprint 2 | `price` (até R$ 6.735) e `freight_value` (até R$ 409,68) documentados via `describe()`; nenhum valor removido — decisão de tratamento fica para quando alguma análise específica precisar |
| Consistência de datas | A verificar no Sprint 2 | Checar se `order_purchase_timestamp` é sempre anterior às demais datas do pedido |
...
Ao criar um novo notebook (modelagem dimensional, análises das trilhas, etc.),
adicionar uma linha nesta tabela seguindo o mesmo padrão:
